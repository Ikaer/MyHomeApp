...
const CODE_VERIFIER_FILE = path.join(ANIME_DATA_PATH, 'oauth_state.json');

function saveCodeVerifier(state: string, verifier: string): void {
  try {
    if (!fs.existsSync(ANIME_DATA_PATH)) {
      fs.mkdirSync(ANIME_DATA_PATH, { recursive: true });
    }
...
async function handleOAuthCallback(req: NextApiRequest, res: NextApiResponse, code: string, state: string) {
  ...
  try {
    ...
    res.redirect('/anime?auth=success');
  } catch (error) {
    console.error('OAuth callback error:', error);
    res.redirect('/anime?auth=error');
  }
}
